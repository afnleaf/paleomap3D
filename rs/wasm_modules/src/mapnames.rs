...
geological era names for each of the 109 maps
ported from frontend/public/maps.js
index 0 = present day, index 108 = earliest cambrian
*/

// (era_name, age_description)
// the title line and the parenthetical line from the JS version
pub const MAP_NAMES: [(&str, &str); 109] = [
...
pub fn get_map_title(index: usize) -> String {
    if index < MAP_NAMES.len() {
        let (era, age) = MAP_NAMES[index];
        format!("{}\n({})", era, age)
    } else {
...
pub fn get_map_title_html(index: usize) -> String {
    if index < MAP_NAMES.len() {
        let (era, age) = MAP_NAMES[index];
        format!("{}<br>({})", era, age)
    } else {
